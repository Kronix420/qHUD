...
namespace PoeHUD.Hud.Preload
{
    public class PreloadAlertPlugin : SizedPlugin<PreloadAlertSettings>
    {
        ...
        private readonly Dictionary<string, PreloadConfigLine> alertStrings, preloadStrings;
            ...
            : base(gameController, graphics, settings)
        {
            ...
            preloadStrings = LoadConfig("config/preloadStrings.txt");
        ...
        private void Parse()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (preloadStrings.ContainsKey(text)) { alerts.Add(preloadStrings[text]); }
                ...
                //Dictionary<string, PreloadConfigLine> Perandus = new Dictionary<string, PreloadConfigLine>
                //{
                ...
                //PreloadConfigLine perandus = Perandus.Where(kv => text
                //    .StartsWith(kv.Key, StringComparison.OrdinalIgnoreCase)).Select(kv => kv.Value).FirstOrDefault();
                //if (perandus != null && Settings.PerandusBoxes)
                //{
                //    alerts.Add(perandus);
                //    continue;
                //}
                ...
                //    .StartsWith(kv.Key, StringComparison.OrdinalIgnoreCase)).Select(kv => kv.Value).FirstOrDefault();
                //if (strongboxes != null && Settings.Strongboxes)
                //{
                //    alerts.Add(strongboxes);
                //    continue;
                //}
                ...
                //};
                //PreloadConfigLine exiles = Exiles.Where(kv => text
                //    .EndsWith(kv.Key, StringComparison.OrdinalIgnoreCase)).Select(kv => kv.Value).FirstOrDefault();
                //if (exiles != null && Settings.Exiles)
                //{
                //    alerts.Add(exiles);
                //}
            }
